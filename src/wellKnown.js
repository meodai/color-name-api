...
export function buildWellKnownHandler({ httpRespond, responseHeaderObj, getOpenApiJSONObject }) {
...
const DEFAULT_OPENAPI_PATH = path.resolve(__dirname, '../color-names-v1-OpenAPI.yml');

export async function initWellKnown({ openApiPath, httpRespond, responseHeaderObj }) {
  ...
  try {
    ...
    console.error('Failed to load OpenAPI spec (via wellKnown module):', specErr);
  }
