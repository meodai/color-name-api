...
const routesToTest = {
  ...
  '/?noduplicates=true&list=basic&values=000000,111111,222222,333333,444444,555555,666666,777777,888888,999999,aaaaaa,bbbbbb,cccccc,dddddd,eeeeee,ffffff,123456,654321,abcdef,fedcba,010101,020202': (response) => {
    // We should get an error response since we're requesting 22 colors but the basic list only has 21
    if (!response.error) {
      throw new Error('Expected an error response for exhausted colors but got a success response');
    }
    ...
    if (response.error.requestedCount !== 22) {
      throw new Error(`Expected requestedCount to be 22 but got ${response.error.requestedCount}`);
    }
    
    // The total count should be 21 (number of colors in basic list)
    if (response.error.totalCount !== 21) {
      throw new Error(`Expected totalCount to be 21 but got ${response.error.totalCount}`);
    }
    
    console.log('✅ Exhausted colors test passed!');
  },
  // Test for requesting exactly the number of colors available (21) in the basic list
  '/?noduplicates=true&list=basic&values=000000,111111,222222,333333,444444,555555,666666,777777,888888,999999,aaaaaa,bbbbbb,cccccc,dddddd,eeeeee,ffffff,123456,654321,abcdef,fedcba,010101': (response) => {
    // This should succeed since we're requesting exactly the number of available colors
    if (response.error) {
      throw new Error(`Got unexpected error when requesting exact number of colors: ${response.error.message}`);
    }
  ...
  '/?values=000000,111111,222222,333333,444444&noduplicates=true&list=short': (response) => {
