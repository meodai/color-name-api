...
async function runTest() {
  ...
  console.log(`Testing with list "${listToTest}" which has ${colorCount} colors`);
  console.log(`Requesting ${testColorCount} colors to trigger the exhausted colors error`);
  ...
  try {
    ...
    if (response.error.requestedCount !== testColorCount) {
      throw new Error(`Expected requestedCount to be ${testColorCount} but got ${response.error.requestedCount}`);
    }
    
    if (response.error.totalCount !== colorCount) {
      throw new Error(`Expected totalCount to be ${colorCount} but got ${response.error.totalCount}`);
    }
