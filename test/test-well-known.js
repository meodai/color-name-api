...
async function run() {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    if (data.schema_version !== 'v1') throw new Error('schema_version is not v1');
    if (!data.api || data.api.type !== 'openapi') throw new Error('api.type is not openapi');
    if (!data.api.url || !data.api.url.endsWith('/openapi.yaml')) throw new Error('api.url does not point to /openapi.yaml');
