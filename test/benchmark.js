...
console.log(`Starting benchmark with ${NUM_COLORS_TO_BENCHMARK} random colors using the '${DEFAULT_LIST_KEY}' list...`);
...
try {
  ...
  // --- End Generate Test Data ---
    ...
    console.error("Error during benchmark setup (FindColors init or data generation):", error);
...
try {
  ...
  const resultsNormal = findColors.getNamesForValues(testColors, false, DEFAULT_LIST_KEY);
  ...
  if (resultsNormal.length !== NUM_COLORS_TO_BENCHMARK) {
      console.warn(`Warning: Normal mode returned ${resultsNormal.length} results, expected ${NUM_COLORS_TO_BENCHMARK}`);
  } else {
  ...
  const resultsUnique = findColors.getNamesForValues(testColors, true, DEFAULT_LIST_KEY);
  console.timeEnd(`[Benchmark] ${NUM_COLORS_TO_BENCHMARK} colors (unique)`);
   // Add check for results length consistency
  if (resultsUnique.length !== NUM_COLORS_TO_BENCHMARK) {
      console.warn(`Warning: Unique mode returned ${resultsUnique.length} results, expected ${NUM_COLORS_TO_BENCHMARK}`);
  } else {
